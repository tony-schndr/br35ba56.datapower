...
class ActionModule(ActionNetworkModule):
    def run(self, tmp=None, task_vars=None):
        ...
        self._config_module = (
            True if module_name in ["config", "files", "action"] else False
        )
        ...
        if persistent_connection == "network_cli":
            ...
            pc.connection = "httpapi"
            pc.network_os = "community.datapower.rest_mgmt"
            pc.remote_addr = provider["host"] or self._play_context.remote_addr
            pc.port = int(provider["port"] or self._play_context.port or 5554)
            pc.remote_user = (
            ...
            if connection is None:
                pc.connection = "network_httpapi"
                pc.network_os = "datapower"
                connection = self._shared_loader_obj.connection_loader.get(
