...
def test_class_name_from_config_with_valid_data():
    data = {
        "class_name": {
        }
    }

    assert class_name_from_config(data) == "class_name"


def test_class_name_from_config_with_invalid_data():
    data = {
        "class_name": {
            "name": "foo"
        },
    ...
    try:
        class_name_from_config(data)
        assert False
    except InvalidConfigDataException:
        assert True


def test_name_from_config_with_valid_data():
    data = {
        "class_name": {
            "name": "foo"
        }
    }
    assert name_from_config(data, 'class_name') == 'foo'


def test_name_from_config_name_key_not_present():
    data = {
        "class_name": {
        }
    }
    try:
        name_from_config(data, 'class_name')
        assert False
    except InvalidConfigDataException:
        assert True


def test_convert_bool_to_on_or_off():
    data = {
        "key1": True,
        "key2": False
    }

    converted_data = convert_bool_to_on_or_off(data)
    assert converted_data == {"key1": "on", "key2": "off"}
